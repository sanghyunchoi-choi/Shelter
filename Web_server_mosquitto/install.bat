...
:: 2. 전달받은 완제품 바이너리 tar 이미지 강제 주입 (소스코드 은닉 핵심)
echo [2/3] 도커 완제품 패키지 이미지 파일 마운트 중... (약 15~30초 소요)
docker load -i shelter-web-image.tar
echo --- 이미지 안착 완료.
echo.

:: 3. 주입된 이미지를 기반으로 모스키토 브로커와 웹 관제 마스터 백그라운드 시동
echo [3/3] 통합 관제 마스터 엔진 및 모스키토 브로커 백그라운드 기동 시작...
docker compose up -d
echo --- 컨테이너 가동 바인딩 완료.
echo.
